...
export const UpdateVotes = ({ article_id , article_votes, comment_id, votes }) => {
  ...
  const handleClickVote = (action) => {
    ...
    patchVotes(comment_id, article_id, action)
    .then((_)=> {
      setIsLoading(false)
    })
    .catch((err) => {
    ...
    return (
      <>
        <div className="flex justify-end p-4 gap-2">
          ...
          <span className="flex items-center justify-center rounded-md bg-gray-100 min-w-[2rem] px-1">{article_id ? article_votes + userVote : votes + userVote}</span>
